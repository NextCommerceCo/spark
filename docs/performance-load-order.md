...
8. Ordered Spark JavaScript modules and Web Components.
9. Page `extrascripts`, inline Spark enhancements, footer app hooks, and `tracking`.
...
- Keep new JavaScript at the bottom by default. The existing script tags are dependency-ordered because page `extrascripts` use globals such as `SparkVariantState`, `SparkCartClient`, and `SparkEvents`.
- Do not add `defer` to the core script stack unless dependent inline `extrascripts` are also moved to a deferred-safe initialization pattern.
