...
- The same script's `--check` mode rejects constructs the platform compiler evaluates rather than passes through: standalone `min()`/`max()`/`clamp()`; CSS filter functions Sass claims as colour built-ins (`invert()`, `saturate()`, `grayscale()`, `opacity()`); and Sass colour functions that are not CSS at all (`lighten()`, `darken()`, `complement()`, `desaturate()`). All fail only at upload time otherwise. Custom-property values are exempt from the built-in-name rules: Sass does not evaluate them.
- **The content scan set is explicit.** `css/input.css` imports Tailwind with `source(none)` and lists exactly four scanned directories: `templates/`, `partials/`, `layouts/`, and `assets/js/`. Tailwind v4 otherwise scans every non-ignored file, so a word in a doc or a code comment that matches a utility name (`collapse`, `grayscale`, `z-70`) compiles a real utility into `main.css` — that is what forced a comment reword in 7e6f446 (#52). If you add a directory that contains class attributes, add an `@source` line for it, update `EXPECTED_SOURCES` in `tests/test_tailwind_source_guard.py`, and rerun `make release`; otherwise its classes silently generate nothing.
